...
func QQMusicSongInfo(id string) (gjson.Result, error) {
	d, err := download.Request{
		URL: fmt.Sprintf(
			...
			%%22comm%%22:{%%22ct%%22:24,%%22cv%%22:0},
			%%22songinfo%%22:{
			%%22method%%22:%%22get_song_detail_yqq%%22,
			%%22param%%22:{%%22song_type%%22:0,%%22song_mid%%22:%%22%%22,%%22song_id%%22:%s},
...
func NeteaseMusicSongInfo(id string) (gjson.Result, error) {
	d, err := download.Request{
		URL: fmt.Sprintf("http://music.163.com/api/song/detail/?id=%s&ids=%%5B%s%%5D", id, id),
	}.JSON()
	if err != nil {
		return gjson.Result{}, err
	}
